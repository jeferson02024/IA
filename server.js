...
function seedEnvKeys() {
  const u = db.prepare('INSERT OR IGNORE INTO config (key,value) VALUES (?,?)');
  if (process.env.GROQ_API_KEY) u.run('global_groq_key', process.env.GROQ_API_KEY);
  if (process.env.GEMINI_API_KEY) u.run('global_gemini_key', process.env.GEMINI_API_KEY);
}
seedEnvKeys();
