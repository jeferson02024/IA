...
app.post('/api/chat', auth, async (req, res) => {
  ...
  try {
    const reply = provider === 'groq'
      ? await callGroq(apiKey, model || 'llama-3.3-70b-versatile', messages, systemPrompt)
      : await callGemini(apiKey, model || 'gemini-2.0-flash', messages, systemPrompt);
    res.json({ reply });
  } catch (err) {
...
app.get('/api/admin/config', auth, role('creator','admin'), (req, res) => {
  ...
  res.json({
    groqKeyMasked: groqKey ? groqKey.slice(0,8)+'••••••••••••' : null,
    geminiKeyMasked: geminiKey ? geminiKey.slice(0,8)+'••••••••••••' : null,
    hasGroqKey: !!groqKey, hasGeminiKey: !!geminiKey,
